...
AdvancedRenderingPipeline::AdvancedRenderingPipeline(const SharedDevice& device,
...

	{
		...
		auto secondDep = SubpassDependency::first()
			.setSrcSubpass(0)
	...
	// Output pass (used for FXAA)
	{
		...
		dsinfoOutput.addBinding(vk::DescriptorType::eCombinedImageSampler,
			vk::ShaderStageFlagBits::eFragment, 1);
		mOutputDescriptorLayout = mDevice->createDescriptorSetLayout(dsinfoOutput);
...
void AdvancedRenderingPipeline::resize(int w, int h) {
	...
	mFboPre =
		mPassPre->createFramebuffer({ mViewDepthPre });

	mFboForward =
		mPassForward->createFramebuffer({ mViewDepth, mViewColor });
	...
	auto forwardSampler = mDevice->createSampler(
		...
		.setAddressModeU(vk::SamplerAddressMode::eMirrorClampToEdge)
		.setAddressModeV(vk::SamplerAddressMode::eMirrorClampToEdge)
		.setAddressModeW(vk::SamplerAddressMode::eMirrorClampToEdge)
		.setMipLodBias(0.0f)
	...
	mForwardDescriptor->writeCombinedImageSampler({ forwardSampler, mViewDepthPre }, 1);
	...
	mOutputDescriptor->writeCombinedImageSampler({ outputSampler, mViewColor }, 0);
}

void AdvancedRenderingPipeline::assignCamera(const camera::SharedGenericCamera& cam,
	bool useCamViewport) {
	mCamera = cam;
...
void AdvancedRenderingPipeline::render(
	...
	//mImageDepthPre->changeLayout(vk::ImageLayout::eGeneral, cmd);
	{
		...
		{
			AdvancedRenderPass rp{ RenderPassType::Opaque, this, cam, mFboForward,
						  pass };
			renderFunc(rp);
		}
	}
