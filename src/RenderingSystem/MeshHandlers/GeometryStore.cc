...
GeometryStore::GeometryStore(lava::SharedDevice device) : mDevice(device) {
...
void GeometryStore::registerGeometry(std::shared_ptr<Geometry> obj,
                                     std::string name) {
    mGeometrys[name] = obj;
}

void GeometryStore::registerGeometryFromData(
    lava::geometry::SharedGeometryData geomData, std::string name) {
    std::shared_ptr<Geometry> sharedGeometryObj =
        std::make_shared<Geometry>(geomData->uploadTo(mDevice));
...
void GeometryStore::registerGeometryFromFileMulti(std::string filePath,
    ...
    auto geometryDataParts = mGeometryImporter.loadMulti<VertexAttributes>(
        filePath, materialIndices, meshNames);

    for (unsigned int i = 0; i < geometryDataParts.size(); ++i) {
        std::shared_ptr<Geometry> sharedGeometryObj =
            std::make_shared<Geometry>(
                geometryDataParts.at(i)->uploadTo(mDevice));

        if (meshNames[i] == "") {
            // @todo Use assimp version which does read the names.
            std::cout << "GeometryStore: registerGeometryFromFileMulti(...) - "
                         "mesh name could not be read."
                      << std::endl
                      << "Falling back to default name." << std::endl;
            std::stringstream ss;
            ss << i;
            meshNames[i] = ss.str();
        }
