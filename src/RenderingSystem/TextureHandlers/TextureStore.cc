...
TextureStore::TextureStore(lava::SharedDevice device)
    : TextureStore(device,
                   std::vector<std::pair<std::string, std::string>>(0)) {}

std::shared_ptr<Texture> TextureStore::getTextureWithName(
    std::string name) const {
    return mTextures.at(name);
}

lava::SharedDescriptorSetLayout TextureStore::getTextureLayout() const {
...
TextureStore::TextureStore(lava::SharedDevice device,
    ...
    createAllTextures(device, imageTexturePathsAndNames);
}
...
void TextureStore::createAllTextures(
    ...
    texlayoutinfo.addCombinedImageSampler(vk::ShaderStageFlagBits::eFragment);
    ...
    for (auto pathAndName : texturePathsAndNames) {
        ...
        lava::SharedImage img =
            UtilsLava::loadCreateAndUploadImgForTexture_default(filePath,
                                                                device);
        ...
        ds->writeCombinedImageSampler({sampler, img->createView()},
                                      0  // binding
        );
