...
void GeometryLoader::loadRawMulti(const std::string& filename,
	...
	std::vector<std::string>& meshNames)
{
	...
	for (auto i = 0u; i < scene->mNumMeshes; ++i)
	{
		...
		for (auto v = 0u; v < vCnt; ++v)
		{
			...
			verticesCurrentMesh.push_back({ pos, normal, tangent, texCoord, colors });
...
void GeometryLoader::printNodeInfo_recursive(const aiScene* scene, const aiNode* node) const
{
	...
	for (auto j = 0u; j < node->mNumMeshes; ++j)
	{
		...
		std::cout << "Node mesh (name/material index): " << mesh->mName.C_Str() << " / " << mesh->mMaterialIndex << std::endl;
	}
	...
	std::cout << "- Node info end -" << std::endl
		<< std::endl;
}
...
void GeometryLoader::loadRawCombined(const std::string& filename,
	std::vector<VertexAttributes>& verticesResult,
	std::vector<uint32_t>& indicesResult)
{
	...
	loadRawMulti(filename, verticesPerMesh, indicesPerMesh, materialIndexPerMesh, namePerMesh);
	...
	std::vector<std::string>& meshNames)
{
	...
	loadRawMulti(filename, verticesPerMesh, indicesPerMesh, meshMaterialIndices, meshNames);

	assert(verticesPerMesh.size() == indicesPerMesh.size());
	assert(verticesPerMesh.size() == meshMaterialIndices.size());
	assert(verticesPerMesh.size() == meshNames.size());

	std::vector<std::shared_ptr<lava::geometry::GeometryData>> geometryDataPointers;
