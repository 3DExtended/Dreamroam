...
lava::SharedImage loadCreateAndUploadImg(const std::string& filePath,
    ...
    unsigned char* pixels = stbi_load(filePath.c_str(), &texWidth, &texHeight,
                                      &texChannels, STBI_rgb_alpha);

    if (pixels == nullptr) {
        lava::log(lava::LogLevel::Error)
            << "Could not load image with title: '" << filePath << "'";
        assert(false);
    }

    auto imgData = lava::ImageData::createFromData(pixels, texWidth, texHeight,
                                                   1,  // depth
...
lava::SharedImage loadCreateAndUploadImgForTexture_default(
    ...
    return loadCreateAndUploadImg(filePath, device,
