...
namespace DCore
{
	...
	{
		/// <summary>
		/// System base is the recommended method for accessing your entities and their components.
		/// The system originates from the core concept of ECS (entity component systems):
		/// You write custom components which can be assigned to entities.
		/// When you want to update those components (or want to read them), you have to create a
		/// system.
		///
		/// For this system you have to specify before hand, which components you try to access.
		/// For example, a RenderingSystem has to access the RenderComponent and the transform component
		/// of all entities. (If an entity doesn't provide both, we don't want to access it in our
		/// RenderingSystem).
		...
		///
		/// Now, in order to act on those selected entities we can implement one of the five
		/// methods: Awake, Start, Update, LateUpdate, Destroy.
		/// A registered system (see the Samples/Systems folder for an example) can now
		/// access the entities like this:
		///
