...
class GeometryStore
{
  ...
  public:
    ...
    void
    registerGeometryFromData(
        lava::geometry::SharedGeometryData geomData,
        std::string name);

    void
    registerGeometryFromFileSingle(
        std::string filePath,
        std::string name);

    void
    registerGeometryFromFileMulti(
        std::string filePath,
...
inline std::shared_ptr<Geometry>
GeometryStore::getGeometryWithName(
    std::string name)
    const
{
...
inline size_t
GeometryStore::getLayerCountWithName(
    std::string name)
    const
{
    auto it = mNumberOfLayersPer.find(name);
    if (it == mNumberOfLayersPer.end())
        return 1;
    return it->second;
}
